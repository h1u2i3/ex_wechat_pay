defmodule ExWechatPay.Utils.PostBody do
  ...
  def add_config_data(body),             do: body |> add_item({:appid, _appid}) |> add_item({:mch_id, _mch_id}) |> add_item({:op_user_id, _mch_id})
                                                  |> add_item({:wxappid, _appid}) |> add_item({:mch_appid, _appid}) |> add_item({:mchid, _mch_id})
  ...
  def validate_body(%PostBody{valid?: true, map: %{transaction_id: _}} = body), do: body |> remove_need([:out_trade_no, :refund_id, :out_refund_no])
  def validate_body(%PostBody{valid?: true, map: %{out_trade_no: _}} = body),   do: body |> remove_need([:transaction_id, :refund_id, :out_refund_no])
  def validate_body(%PostBody{valid?: true, map: %{out_refund_no: _}} = body),  do: body |> remove_need([:refund_id, :out_trade_no, :transaction_id])
  def validate_body(%PostBody{valid?: true, map: %{refund_id: _}} = body),      do: body |> remove_need([:out_refund_no, :out_trade_no, :transaction_id])
  ...
  defp remove_item(body, keys),           do: body |> struct(map: body.map |> Map.drop(keys))
  defp remove_required(body, keys),       do: body |> struct(required: body.required -- keys)
  defp key_miss_message(keys),            do: "still need post body keys: #{Enum.join(keys, " ")}"
  defp remove_need(body, keys),           do: body |> remove_item(keys) |> remove_required(keys)
end
